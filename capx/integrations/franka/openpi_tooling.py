...
class FrankaLiberoOpenPIToolMixin:
    ...
    def get_openpi_action_chunk(
        ...
        Args:
            ...
            prompt: Optional task instruction override. Prefer leaving this as None so OpenPI sees
                the exact current LIBERO task language, which matches its training/eval path.
            host: Host of the OpenPI websocket server.
            port: Port of the OpenPI websocket server.
    ...
    def execute_openpi_step(
        ...
        This is usually the safest OpenPI execution primitive on LIBERO-PRO variants because it
        forces a re-observe / replan loop after one short-horizon VLA step.
        """
    ...
    def execute_openpi_plan(
        ...
        Args:
            ...
            execute_subgoals: How many interpreted subgoals to execute from the planned chunk.
                On LIBERO-PRO, prefer 1-2 unless the path is clearly safe.
            resize_size: Square image size used for OpenPI preprocessing. Default is 224.
            prompt: Optional task instruction override. Prefer leaving this as None so OpenPI sees
                the exact current LIBERO task language, which matches its training/eval path.
            host: Host of the OpenPI websocket server.
